...
def install():
    ...
    if len(sys.argv) == 3 and (sys.argv[2]) == "dev":
        exit(os.WEXITSTATUS(os.system("pip install -r requirements-dev.txt")))
    else:
        exit(os.WEXITSTATUS(os.system("pip install -r requirements.txt")))


def run():
    """Starts the backend"""
    exit(os.WEXITSTATUS(os.system("python -m mod")))


def run_tests():
    """Runs pytest and provides coverage information when '--cov' is provided"""
    if len(sys.argv) >= 3:
        exit(os.WEXITSTATUS(os.system(f"pytest mod {' '.join(sys.argv[2:])}")))
    else:
        exit(os.WEXITSTATUS(os.system("pytest mod")))


def run_typecheck():
    """Runs typecheck"""
    exit(os.WEXITSTATUS(os.system("mypy mod")))


def run_linter():
    """Runs linter"""
    exit(os.WEXITSTATUS(os.system("pylint mod")))


def format():
    """Formats API as specified from GitHub CI"""
    exit(os.WEXITSTATUS(os.system("black . & isort .")))


def build():
    """Builds API as specified from GitHub CI"""
    exit(os.WEXITSTATUS(os.system("mypy mod & pylint mod & pytest mod --cov")))
...
def create_migration():
    ...
    status = os.WEXITSTATUS(os.system("alembic revision --autogenerate"))
    print(
...
def dump_api():
    """Dumps API"""
    exit(os.WEXITSTATUS(os.system("PYTHON_ENV=production python3 -m mod.__dumpapi__")))
